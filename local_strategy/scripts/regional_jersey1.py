...
class Pose():
	...
	DropBall 		= [0, 0, 0]
	KickOffHome 	= [-1000, -200, 90]
	KickOffAway 	= [-3000, 500, 90]
	GoalKickHome 	= [0, 0, 0]
	GoalKickAway 	= [0, 0, 0]
	CornerHome 		= [0, 0, 0]
	CornerAway 		= [0, 0, 0]
...
def WaitBall(time_out=30):
	global action_stats, jersey2_state

	time_count = 0
	while jersey2_state != RobotState.WaitFriend and enable != Enable.Stop:
		if rospy.is_shutdown():
			break
		if time_count >= time_out:
			break
...
# # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # #
	...
	while not rospy.is_shutdown():
		if enable == Enable.Stop:
			...
			SetAbsPose([robot_pose.x, robot_pose.y, GetAngle([jersey2_pose.x, jersey2_pose.y+250])])
			ShootBall(18)
			...
			SetAbsPose([-3300, 3650, 84])	# [-3000, 3500, 90]
			sleep(1)
			if front_dummy_detected and (front_dummy_point.y <= 0.57):
			...
			if not dummy[3]:
				...
				SetAbsPose([-2750, 3950, 84])	# [-2250, 3750, 90]
				InitFL([-1750, 4050, 90])
				sleep(1)
				if front_dummy_detected and (front_dummy_point.y <= 0.75) and (abs(front_dummy_point.w) <= 20):
					dummy[8] = True
					if not dummy[2]:
						SetAbsPose([-1750, 3000, 90])	# [-1750, 3000, 90]
						InitF([-1750, robot_pose.y, 90])
					else:
						SetAbsPose([-3000, 3800, 90])	# [-3000, 3800, 90]
			else:
				SetAbsPose([robot_pose.x, robot_pose.y-300, robot_pose.theta])
				SetAbsPose([-2500, 3250, 86])	# [-2250, 3000, 90]
				InitF([-1750, robot_pose.y, 90])
			WaitUntilFriendReady()
			...
			if not dummy[3]:
				...
				SetAbsPose([-2250, 3650, 90])	# [-2250, 3750, 90]
				InitFL([-1750, 4050, 90])
			...
			if KeeperPosition() >= -175:
				...
				SetAbsPose([robot_pose.x, robot_pose.y, GetAngle([400, 6000])])
			ShootBall(36)
